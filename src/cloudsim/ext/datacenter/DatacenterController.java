...
public class DatacenterController extends DatacenterBroker implements GeoLocatable,
	...
	private void submitWaitingCloudlet(){
		int nextAvailVM = loadBalancer.getNextAvailableVm();
                //System.out.println("----Next Available VM from SWC : ------ " + nextAvailVM);
		if ((nextAvailVM != -1) && (waitingQueue.size() > 0)){//System.out.println("Inside first if statement****");
	...
	private void submitNewCloudlet(InternetCloudlet cl) {
		...
		if (loadBalancer == null){
			loadBalancer = new RoundRobinVmLoadBalancer(vmStatesList,this);
		}
		
		int nextAvailVM = loadBalancer.getNextAvailableVm();
                //System.out.println("----Next Available VM from SNC : ------ " + nextAvailVM);
					
		if (nextAvailVM == -1){//System.out.println("Inside if block!!!!!!!!!!");
	...
	private void submitCloudlet(InternetCloudlet cl, int vmId) {
		...
		cl.setUserID(this.get_id());
                //System.out.println("----Before the array statement-----");
		int dest = vmMapping[vmId];
                //System.out.println("----After the array statement-----");
