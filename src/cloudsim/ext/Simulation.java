...
public class Simulation extends BaseCloudSimObservable implements Constants {
		...
		for (DataCenterUIElement d : dataCenters) {
			if (d.isAllocated()){
				...
				int brokerId = controller.get_id();
                                if(!StaticVars.VmMap.containsKey(d.getName())) {
                                    throw new RuntimeException("EXCEPTION : " + d.getName() + " doesn't exist in StaticVars.vmMap. Inside method 'runSimulation'");
                                }
                                int vmCount = 0;
                                LinkedList<VmAllocationUIElement> list = StaticVars.VmMap.get(d.getName());                      
                                for(VmAllocationUIElement vm : list) {
                                    vmCount = vmCount + vm.getVmCount();
                                }
				//vmlist = createVM(brokerId, d.getVmAllocation().getVmCount()); // Original
	...
	public void setLoadBalancePolicy(String loadBalancePolicy) {
		this.loadBalancePolicy = loadBalancePolicy;
                //System.out.println("$$$$$$$$$$Load Balancing Policy$$$$$$$ : " + loadBalancePolicy);
	}
