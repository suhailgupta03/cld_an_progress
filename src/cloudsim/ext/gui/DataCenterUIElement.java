...
public class DataCenterUIElement extends SimulationUIElement implements Serializable {
	...
	public void setVmAllocation(VmAllocationUIElement vmAllocation) {
		this.vmAllocation = vmAllocation;
                ...
                if(StaticVars.VmMap.containsKey(this.vmAllocation.getDc().toString())) {
                    LinkedList<VmAllocationUIElement> list = StaticVars.VmMap.get(this.vmAllocation.getDc().toString());
                    list.add(this.vmAllocation);
                    StaticVars.rowMap.put(StaticVars.currRow, list.size()-1);
                }else {
                    LinkedList<VmAllocationUIElement> list = new LinkedList<VmAllocationUIElement>();
                    list.add(this.vmAllocation);
                    StaticVars.VmMap.put(this.vmAllocation.getDc().toString(), list);
                    StaticVars.rowMap.put(StaticVars.currRow, list.size()-1);
                }
	}
