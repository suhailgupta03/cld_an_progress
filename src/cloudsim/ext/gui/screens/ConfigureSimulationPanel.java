...
import static cloudsim.ext.Constants.DEFAULT_DATA_CENTER_NAME;
...
public class ConfigureSimulationPanel extends JPanel 
	...
	private JComboBox cmbLoadBalancingPolicy;
        ...
        private DepConfList depconfList;
        private Map<Object,Object> confMap;
        private Map<Object,Object> confMapFirstRow = new HashMap<Object,Object>();
        private int oldVMCount = 0;
        private long oldBWCount = 0L;
        private int oldMemorySize = 0;
        private long oldImageSize = 0L;
        
        private boolean vmCountChanged = false;
        private boolean bwChanged = false;
        private boolean memorySizeChanged = false;
	...
	private JPanel createAdvancedTab(){
		...
		compW = 240;
		compH = 30;
		JLabel lblLoadBalancing = new JLabel("<html>Load balerficeribviancing policy<br/>" +
	...
	private JComboBox initDcCombo(){
		if (dcCombo == null){
			dcCombo = new JComboBox();
                        // Change
//                        vmCombo = new JComboBox();
                        //-----
		} else {
			dcCombo.removeAllItems();
                        // Change
                        //vmmCombo.removeAllItems();
                        //--------
		}
		
		for (DataCenterUIElement dc : dataCenterList){
			dcCombo.addItem(dc);//vmCombo.addItem(dc);
	...
	private class VmTableModel extends AbstractListTableModel<VmAllocationUIElement> {
		...
		protected void setValueAtInternal(Object value, int row, int col) {
			...
			if(defaultSet) {
                            ...
                            VmAllocationUIElement vmDeafult = new VmAllocationUIElement(dataCenter, 
                                                                                            Constants.DEFAULT_VM_COUNT, 
                                                                                            Constants.DEFAULT_VM_IMAGE_SIZE, 
                                                                                            Constants.DEFAULT_VM_MEMORY, 
                                                                                            Constants.DEFAULT_VM_BW);
                            
                            confMapFirstRow.put("VmAllocationUIElement", vmDeafult);
                            DepConfList.setMap(0, confMapFirstRow); // Insert the default row 
                            defaultSet = false;
                        }
			...
			case 0:
				...
				vm.setDc(selectedDc);   
                                StaticVars.currRow = row;
                                StaticVars.modVm = vm;
				selectedDc.setVmAllocation(vm);
                                confMap = new HashMap<Object,Object>();
                                confMap.put("VmAllocationUIElement", vm);
				break;
			case 1:
                                StaticVars.currRow = row;
                                StaticVars.currentDataCenter = vm.getDc().toString();
                                StaticVars.modVm = vm;
				vm.setVmCount((Integer) value);
                                confMap = new HashMap<Object,Object>();
                                confMap.put("VmAllocationUIElement", vm);
                                DepConfList.setMap(row, confMap);
                                vmCountChanged = true;
				break;
			case 2:
                                StaticVars.currRow = row;
			...
			case 4:
				...
				break;
                        ...
                        if(!vmCountChanged && !imageSizeChanged && !memorySizeChanged && !bwChanged) {                            
                            DepConfList.setMap(row,confMap); // called only for 'case 0'
                        }
